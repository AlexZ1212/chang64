...
function search(g, maxDepth, timeMs) {
  ...
  /* Position deja terminee au moment ou on appelle search() : la detection
     mat/pat existe bien plus bas (voir alphabeta), mais seulement comme
     valeur de retour d'un appel recursif dont le resultat n'est jamais
     recueilli par bestScore, qui n'est mis a jour que dans la boucle des
     coups plus bas. Sans coup a jouer, cette boucle ne tourne jamais, et
     bestScore restait a 0 : un mat etait alors note comme un pat. Trouve
     via l'exercice #MFLCX (p346), ou Dxe2# notait 0 alors qu'il gagne la
     partie sur-le-champ. */
  if (g.moves().length === 0) return { move: null, score: g.inCheck() ? -MATE : 0 };
