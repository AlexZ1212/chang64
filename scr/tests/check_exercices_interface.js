...
setTimeout(async()=>{
  ...
  await new Promise(r=>setTimeout(r,400));
  const withExplain=w.eval('PUZZLES.find(p=>p.theme==="Winning capture"&&p.explain&&p.explain.piece)');
  T("un exercice avec detail exploitable existe", !!withExplain);
  w.eval('puzzle=PUZZLES.find(p=>p.theme==="Winning capture"&&p.explain&&p.explain.piece); loadPuzzle();');
