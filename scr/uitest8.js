...
(async()=>{
  ...
  console.log("\nENDGAME TRAINER");
  /* Les finales vivent dans Exercices, pas Defis (voir refreshCurrentMode) :
     ce test visait le mauvais onglet, et ne marchait que parce qu'un vieux
     bloc de code dans le gestionnaire de Defis demarrait une finale au
     hasard en arrivant sur cet onglet — au prix d'un vrai bug (un
     Roi+Dame contre Roi apparaissait sur l'echiquier partage sans aucune
     puce ni statut visible pour l'expliquer). Ce bloc mort a ete retire ;
     le test doit desormais demarrer une finale explicitement, comme le
     ferait vraiment un joueur. */
  click($("tab-puzzles")); await wait(600);
  T("puzzles pane shown", !$("pane-puzzles").className.includes("hide"));
