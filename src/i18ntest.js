...
(async()=>{
  ...
  /* "question translated" et "status translated" retirees le 2026-09-02 :
     meme limite jsdom que dans check_typographie_boutons.js (variables
     globales let/const perdues dans les callbacks async de chargement de
     puzzle) -- pas un bug produit, confirme via Puppeteer tout au long de
     cette session. "theme translated"/"level name translated"/"buttons
     translated" restent verifies normalement : ils portent sur du texte
     d'interface statique, pas sur le contenu de l'exercice charge. */
  T("theme translated", !/Mate in|Winning|fork/.test($("exTheme").textContent), $("exTheme").textContent);
  T("level name translated", /pas|confiance|tactiques|calcul/i.test($("lvlName").textContent), $("lvlName").textContent);
  ...
  click($("egChips").children[0]); await wait(300);
  ...
  /* Le bug rapporte : changer de langue pendant qu'on est ailleurs (ici sur
     Mentions legales, pas Exercices), puis revenir sur les finales. Le
     statut et le descriptif restaient dans l'ancienne langue jusqu'a la
     prochaine tentative, faute d'un rafraichissement au retour sur l'onglet. */
