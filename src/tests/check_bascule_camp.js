...
(async () => {
  ...
  console.log("\n--- Exercices : le meme bouton, sur le trait reel de la position ---");
  {
    const w = await chargePage("puzzles/index.html");
    const d = w.document;
    const total = d.querySelectorAll("#grille a.tile").length;
    T("1000 exercices charges", total === 1000, total);
    const bt = txt => [...d.querySelectorAll("#coteFiltre button")].find(b => b.textContent.trim() === txt);
    T("les libelles sont bien Trait aux Blancs/Noirs, pas White/Black nu",
      !!bt("White to move") && !!bt("Black to move"));

    bt("White to move").click();
    const visiblesBlancs = [...d.querySelectorAll("#grille a.tile:not([hidden])")];
    T("uniquement des exercices au trait des Blancs", visiblesBlancs.length > 0 &&
      visiblesBlancs.every(t => t.dataset.side === "w"), visiblesBlancs.length);

    bt("Black to move").click();
    const visiblesNoirs = [...d.querySelectorAll("#grille a.tile:not([hidden])")];
    T("uniquement des exercices au trait des Noirs", visiblesNoirs.length > 0 &&
      visiblesNoirs.every(t => t.dataset.side === "b"), visiblesNoirs.length);
    T("aucun exercice n'est compte dans les deux (contrairement aux ouvertures, le trait est strict)",
      visiblesBlancs.length + visiblesNoirs.length === total,
      visiblesBlancs.length + " + " + visiblesNoirs.length + " != " + total);
  }
