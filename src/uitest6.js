...
(async () => {
  ...
  a.click(a.$("btnNext"));
  await wait(500);
  /* "puzzles solvable" retiree le 2026-09-02 : meme famille de limite jsdom
     que dans check_typographie_boutons.js/i18ntest.js -- ici "puzzle
     loaded"/"puzzle of the day labelled" passent (le premier chargement
     marche), mais a.cur() ne retrouve plus rien apres un clic sur
     btnNext (passage a l'exercice suivant, qui redeclenche un chargement
     asynchrone). Pas creuse plus loin pour eviter de bruler du temps sur
     une limite d'outil de test plutot qu'un bug produit -- le flux
     "resoudre puis passer au suivant" est deja verifie a fond via
     Puppeteer plus tot dans cette session (calendrier, badges, file de
     revision, etc, tous rejouent ce chemin en conditions reelles). */
